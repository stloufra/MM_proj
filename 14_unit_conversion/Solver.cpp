...
void Solver::initialization_eq(double rho0, double ux0, double uy0)
{
    for(int j=0; j < Ny; j++)
  {
    for(int i=0; i < Nx; i++)
...
void Solver::output_VTK(int s,int plot_every)
{
    ...
    for(int j=0; j < Ny; j++)
    {
        for(int i=0; i <Nx; i++)
        {
            out_file <<ux(j,i)*Cl/Ct<<" "<<uy(j,i)*Cl/Ct<<" 0\n";
        }
    }
...
void Solver::convert_to_lattice(double L_fyz, double U_fyz, double rho_fyz, double ny_fyz)
{
