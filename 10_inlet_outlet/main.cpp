...
const double uw1 = 0.0;
const double uw2 = 0.0;
const double Re = 400.0;
const double rho_in = 1.3;
const double rho_out = 0.8;
...
double tau;
double nu;
const int plot_every = 25; //25 for 5000
...
{   
    ...
    for (j=0; j<=Ny; j++)
    {
        for(k=0;k<Nvel;k++)
        {   
            feq = f_equilibrium(rho [j][Nx], ux[j][Nx], uy[j][Nx], k);
            f_post_col[j][0][k]=weight[k]*(rho_in+3*(cx_pos[k]*ux[j][Nx]+cy_pos[k]*uy[j][Nx]))+(f[j][Nx][k]-feq);
        }
    }
    ...
    for (j=0; j<=Ny; j++)
    {
        for(k=0;k<Nvel;k++)
        {   
            feq = f_equilibrium(rho [j][1], ux[j][1], uy[j][1], k);
            f_post_col[j][Nx][k]=weight[k]*(rho_out+3*(cx_pos[k]*ux[j][1]+cy_pos[k]*uy[j][1]))+(f[j][1][k]-feq);
